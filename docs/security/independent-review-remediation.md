...
|   2 | Harden signing boundary         | Outstanding                | Current arbitrary-byte signer is documented as prohibited for production; replace with typed intent + KMS/HSM.                                                                                   |
|   3 | Copy execution state machine    | Outstanding                | Current worker is a prototype and disabled executor is default; persist full transition machine and recovery.                                                                                    |
...
|   9 | Branded financial types         | Outstanding                | Existing code still uses authoritative JavaScript numbers in multiple paths.                                                                                                                     |
...
|  11 | Idempotent worker jobs          | Partial                    | A leader event ID exists, but durable execution uniqueness/status/retry metadata is absent.                                                                                                      |
|  12 | Transactional outbox            | Outstanding                | No outbox table/publisher/consumer contract exists.                                                                                                                                              |
|  13 | Continuous reconciliation       | Outstanding                | No durable venue/local reconciliation table and workflow exists.                                                                                                                                 |
...
|  19 | Relational constraints          | Partial                    | PK/FK/unique indexes exist; financial check constraints and comprehensive uniqueness remain.                                                                                                     |
|  20 | Double-entry ledger             | Outstanding                | No internal balanced ledger exists.                                                                                                                                                              |
|  21 | Immutable audit records         | Outstanding                | No append-only, tamper-evident audit chain exists.                                                                                                                                               |
...
|  42 | Migration validation            | Outstanding                | Add empty/prior-schema/concurrent-version migration CI and forward-repair policy.                                                                                                                |
